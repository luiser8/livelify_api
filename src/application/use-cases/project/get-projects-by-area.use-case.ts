...
export interface GetProjectsByAreaRequest {
...
@Injectable()
export class GetProjectsByAreaUseCase {
  constructor(
  ...
  async execute(
    request: GetProjectsByAreaRequest,
  ): Promise<GetProjectsByAreaResponse> {
    const lifeWheelAreaId = LifeWheelAreaId.fromString(request.lifeWheelAreaId);

    // 1. Obtener todos los proyectos del área
    const projects =
      await this.gtdProjectRepository.findByLifeWheelAreaId(lifeWheelAreaId);
